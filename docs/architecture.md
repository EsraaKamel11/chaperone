...
|---|---|---|
| Out-of-process command hook | A `PreToolUse` hook in a separate process, `tools/policy_hook.py` | **The pure half only, and all of it**: the unconsumed-key check, act-classes, citation validation and tripwires. Never the checker. |
