...
flowchart TD
    ...
    denial["BLOCK content:advises_on_merits. The class is named, and the send_message tool is never entered."]
...
`tools/diagram.py` the same way: each participant names a symbol this build resolved, and each
solid arrow is a call found in an AST. The inbound question, the drafted body, the class and the
quoted span are read out of that file rather than retyped here. Watch the participant nothing ever
reaches.
