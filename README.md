...
looks right. The boxes that name no function -- the draft, the denial, the queue, the log -- are
prose, and are marked as such by naming nothing.
...
flowchart TD
    ...
    denial["BLOCK, the class named. In this scene content:advises_on_merits fired, and the send_message tool is never entered."]
    ...
    draft -->|"the permission lane is handed the same one"|chokepoint
    ...
    engine -->|"findings, and a disposition derived from them"|denial
    denial -->|"routes where"|destination
    chokepoint -->|"asks where, and never types a queue name"|destination
