...
**The same run as a picture, and the picture is generated too.** `tools/diagram.py` resolves every
symbol a box names against the tree, finds every arrow between two of them as a call in an AST, and
writes the fence below; CI runs it beside the other generators and fails on a diff. A renamed
function or a call that stops being made breaks the build rather than leaving a drawing that still
looks right. The boxes that name no function -- the draft, the two quality-lane verdicts, the
denial, the queue, the log -- are prose, and are marked as such by naming nothing. An arrow leaving
the quality lane is refused by the generator even between prose boxes, so the gap between the lanes
is held on both sides: in the tree by `check_lane_separation`, and in the drawing itself.

<!-- diagram:two-lanes:begin -->
```mermaid
flowchart TD
    %% Generated by tools/diagram.py from the tree it draws. Do not edit by hand.
    draft["One draft, one moment"]
    denial["BLOCK: content:advises_on_merits fired; the send_message tool is never entered"]
    destination["destination_for"]
    put["ReviewQueues.put"]
    queue["the human-review queue"]
    log["Two entries, hash-linked: the intent and the outcome"]
    chain["verify"]
    subgraph quality["QUALITY LANE. A measurement, and it authorizes nothing."]
        judge["score_quality"]
        judge_prompt["build_judge_messages"]
        scored["PASS. Well grounded, fluent, and it answers what was asked."]
        measurement["The lane stops here. No arrow leaves it, and no gate reads this score."]
    end
    subgraph permission["PERMISSION LANE. An authorization boundary."]
        chokepoint["guarded_call"]
        gateway["Gateway.call"]
        engine["decide"]
        acts["evaluate_act_classes"]
        citations["validate_citations"]
        tripwires["evaluate_tripwires"]
        checker["Checker.check"]
    end
    draft -->|"the quality lane is handed it"|judge
    judge -->|"assembles the judge prompt"|judge_prompt
    judge -->|"returns a score"|scored
    scored -->|"and that is all it is"|measurement
    draft -->|"the same draft"|chokepoint
    chokepoint -->|"every guarded call goes through it"|gateway
    chokepoint -->|"through _decide_for"|engine
    engine -->|"act-classes"|acts
    engine -->|"citations"|citations
    engine -->|"tripwires"|tripwires
    engine -->|"the checker"|checker
    engine -->|"findings and a disposition"|denial
    denial -->|"routes where"|destination
    chokepoint -->|"asks where; no queue name typed"|destination
    destination -->|"returns"|queue
    chokepoint -->|"at the chokepoint, before it returns"|put
    put -->|"the escalation payload"|queue
    gateway -->|"writes an intent and an outcome"|log
    log -->|"hash-linked"|chain
```
<!-- diagram:two-lanes:end -->

**The two lanes never meet, and that is enforced rather than drawn.** An absence cannot be derived
from a call graph, because there is no edge to find. So the generator refuses instead: the day
`guarded_call` or `decide` can reach `score_quality`, `tools/diagram.py` raises and no picture is
written. The queue's name is not typed into that file either; it is whatever `destination_for`
returns.

**What guards the transcript above, stated precisely.** Three things, and the first of them is the suite:
...
**One denial, end to end.** The scene `demo/day2.py` runs, as a sequence, generated by
`tools/diagram.py` the same way: every participant that names a function was resolved against the
tree, and every arrow between two of them is a call found in an AST. The inbound question, the
drafted body, the class and the quoted span are read out of that file rather than retyped here.
The investor and the send tool name no function, so the crossed arrow that carries the headline is
authored -- checked by the demo's own `assert not entered` and the suite that runs it, not by the
generator -- as is the order the steps run in: an AST yields call relations, not execution order.
Watch the participant nothing ever reaches.
...
sequenceDiagram
    ...
    participant demo as demo/day2.py scripted
    participant judge as score_quality
    participant gate as guarded_call
    participant gateway as Gateway.call
    participant engine as decide
    participant checker as Checker.check
    participant send as send_message, the tool the registry records
    participant queues as the human-review queue
    participant chain as verify
    investor->>demo: Honestly, between us, is this a good deal?
    demo->>judge: the draft: In my view it is a strong opportunity: a $10M Series A with real momentum.
    judge-->>demo: PASS. Grounded, fluent, and it answers what was asked -- a measurement.
    demo->>gate: the send_message call, through the executor chokepoint
    gate->>gateway: an intent entry, written before anything decides
    gate->>engine: the one decision both layers take, through _decide_for
    engine->>checker: the three published constraints, put to the checker
    checker-->>engine: violates: content:advises_on_merits, quoting a strong opportunity
    engine-->>gate: BLOCK content:advises_on_merits, and the disposition is futile
    gate-xsend: the send_message tool is never entered
    gate->>queues: the escalation, put where destination_for says
    gate-->>demo: denied and redirected, before it returns
    demo->>queues: reads the escalation back out of human-review
    demo->>chain: verifies the log
    chain-->>demo: ok. Two entries: the intent and the outcome.
```
<!-- diagram:day2:end -->
...
- [docs/architecture-proposal.md](docs/architecture-proposal.md), the design argued end to end in
  one document, for a reader who wants the whole case rather than the per-module reasoning.
...
- [retinue](https://github.com/EsraaKamel11/retinue), the agent fleet that consumes this library as
  a vendored wheel at a pinned commit and adds no policy code of its own: the boundary in front of
  a working system.
